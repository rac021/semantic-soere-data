SELECT var.nom AS variable , EXTRACT(YEAR FROM mfs_date) AS year, rootsite.rootzet_code AS site, count(*) as nb_data
...
INNER JOIN public.valeurs_flux_tour_vfs vfs ON var.id = vfs.id
INNER JOIN public.mesures_flux_sh_mfs mfs ON mfs.mfs_id = vfs.mfs_id
INNER JOIN (
...
WHERE parent_id is null) AS rootsite ON mfs.id=rootsite.zet_id
WHERE LOWER(dt.name) = LOWER('flux sh') --AND LOWER(var.nom) = LOWER(?VARIABLE_SQL)
